...
def process_and_save_image(image_source, output_dir_base, article_dir=""):
    """
    ...
    Returns paths relative to site root (e.g. 'assets/images/file.webp').
    """
    if not image_source:
        return None
    ...
    try:
        ...
        output_dir_images = os.path.join(output_dir_base, IMAGE_ASSETS_DIR)
...
def process_author_photo(photo_path, output_dir_base):
    """
    Processes an author's photo, handling both local paths and URLs.
    Returns paths relative to site root (e.g. 'assets/images/authors/file.webp').
    """
    if not photo_path:
        return None

    output_dir_images = os.path.join(output_dir_base, IMAGE_ASSETS_DIR, "authors")
    os.makedirs(output_dir_images, exist_ok=True)
...
def pre_process_article_media(md_file_info, output_dir_base):
    """
...
def process_audio(audio_path, output_dir_base):
    ...
    (relative to site root, e.g. 'assets/audio/file.mp3').
    """
    ...
    try:
        ...
        output_dir_audio = os.path.join(output_dir_base, AUDIO_ASSETS_DIR)
...
TRANSLATIONS = {
    ...
    "metodo_page": {
        ...
        "p2_3": {
            "it": "Prima di dire cosa entra nel processo, vale la pena dire cosa non entra. Uso soltanto fonti già pubbliche, ciascuna con un URL verificabile, e i testi delle interviste solo dopo che l'intervistato li ha concordati, sapendo che nella produzione di tutto il materiale mi avvalgo di strumenti di intelligenza artificiale. Nessuna confidenza passa di lì, e nessun dato personale che non sia destinato alla pubblicazione. A chi legge e si chiede cosa succede ai materiali che condivido con Perplexity, Claude o NotebookLM devo la stessa trasparenza che chiedo alle aziende di cui scrivo: dove possibile ho disattivato l'utilizzo dei miei contenuti per l'addestramento, e i dati che riguardano me li tengo online apposta, repository compreso, perché le prove di cui parlo restino controllabili da chiunque voglia rifare il percorso.",
            "en": "Before explaining what enters the process, it is worth pointing out what does not enter it. I only use sources that are already public, each with a verifiable URL, and interview texts only after the interviewee has agreed to them, knowing that I use artificial intelligence tools in producing all materials. No confidential information passes through them, nor any personal data not intended for publication. To readers wondering what happens to the materials I share with Perplexity, Claude, or NotebookLM, I owe the same transparency I expect from the companies I write about: wherever possible, I have disabled the use of my content for model training, and I deliberately keep data concerning myself online—including the repository—so that the evidence I reference remains auditabile for anyone wishing to trace my steps.",
            "es": "Antes de explicar qué entra en el proceso, vale la pena señalar qué no entra en él. Solo utilizo fuentes que ya son públicas, cada una con una URL verificable, y los textos de las entrevistas únicamente después de que la persona entrevistada los haya acordado, sabiendo que en la producción de todo el material recurro a herramientas de inteligencia artificial. Ningún secreto pasa por ahí, ni dato personal alguno que no esté destinado a la publicación. A quienes leen y se preguntan qué ocurre con los materiales que comparto con Perplexity, Claude o NotebookLM, les debo la misma transparencia que pido a las empresas sobre las que escribo: siempre que ha sido posible he desactivado el uso de mis contenidos para el entrenamiento, y los datos que me conciernen los mantengo en línea adrede, incluido el repositorio, para que las pruebas de las que hablo sigan siendo auditables por cualquiera que desee comprobar el recorrido.",
            "fr": "Avant d'expliquer ce qui entre dans le processus, il convient de préciser ce qui n'y entre pas. Je n'utilise que des sources déjà publiques, chacune dotée d'une URL vérifiable, et les textes des entretiens uniquement après que l'interviewé les a validés, sachant que dans la production de tout le matériel j'utilise des outils d'intelligence artificielle. Aucune confidence ne passe par là, ni aucune donnée personnelle qui ne soit pas destinée à la publication. À vous qui lisez et vous demandez ce qu'il advient des éléments que je partage avec Perplexity, Claude ou NotebookLM, je dois la même transparence que celle que je demande aux entreprises sur lesquelles j'écris : lorsque cela était possible, j'ai désactivé l'utilisation de mes contenus pour l'entraînement, et les données qui me concernent, je les conserve en ligne à dessein, y compris le dépôt, pour que les preuves dont je parle restent vérifiables par quiconque souhaite refaire le chemin.",
            "de": "Bevor erklärt wird, was in den Prozess einfließt, lohnt es sich festzuhalten, was nicht einfließt. Ich verwende ausschließlich bereits öffentliche Quellen, jede mit einer verifizierbaren URL, und Interviewtexte erst, nachdem die befragte Person ihnen zugestimmt hat – im Wissen, dass ich bei der Erstellung des gesamten Materials KI-Werkzeuge einsetze. Keine vertraulichen Informationen gelangen dorthin und keine personenbezogenen Daten, die nicht zur Veröffentlichung bestimmt sind. Allen Leserinnen und Lesern, die sich fragen, was mit den Materialien geschieht, die ich mit Perplexity, Claude oder NotebookLM teile, schulde ich dieselbe Transparenz, die ich von den Unternehmen verlange, über die ich schreibe: Wo immer möglich, habe ich die Nutzung meiner Inhalte für das Modelltraining deaktiviert, und die Daten, die mich betreffen, halte ich bewusst online – einschließlich des Repositories –, damit die Belege, von denen ich spreche, für jeden nachvollziehbar bleiben, der den Weg überprüfen möchte."
        },
        "h2_3": {
        ...
        "p4_3": {
            "it": "Una precisazione, perché è facile fraintendere questa distinzione. Non significa che ogni frase sia nata dalla mia penna: significa che ogni frase pubblicata è mia, nel senso pieno della parola. Se qualcosa che ho pubblicato viene contestato, non leggerete mai che l'ha scritto Claude. Il testo è ciò che penso e ciò che avrei scritto, anche dove è passato attraverso una riscrittura, ed è esattamente per questo che il controllo finale resta mio.",
            "en": "A clarification, as it is easy to misunderstand this distinction. It does not mean that every single sentence originated from my pen: it means that every published sentence is mine, in the fullest sense of the word. If something I publish is challenged, you will never read that Claude wrote it. The text reflects what I think and what I would have written, even where it went through a rewrite, and that is precisely why final control remains mine.",
            "es": "Una aclaración, porque es fácil malinterpretar esta distinción. No significa que cada frase haya nacido de mi pluma: significa que cada frase publicada es mía, en el sentido pleno de la palabra. Si algo de lo que he publicado es cuestionado, jamás leeréis que lo escribió Claude. El texto es lo que pienso y lo que habría escrito, incluso allí donde ha pasado por una reescritura, y es exactamente por eso que el control final sigue siendo mío.",
            "fr": "Une précision, car il est facile de mal interpréter cette distinction. Cela ne signifie pas que chaque phrase soit nées sous ma plume : cela signifie que chaque phrase publiée est la mienne, au sens plein del du terme. Si un propos publié est contesté, vous ne lirez jamais que c'est Claude qui l'a écrit. Le texte reflète ce que je pense et ce que j'aurais écrit, même là où il a fait l'objet d'une réécriture, et c'est exactement pour cela que le contrôle final me revient.",
            "de": "Eine Klarstellung, da diese Unterscheidung leicht missverstanden werden kann. Es bedeutet nicht, dass jeder Satz aus meiner eigenen Feder stammt: Es bedeutet, dass jeder veröffentlichte Satz im vollsten Sinne des Wortes mein eigener ist. Sollte etwas von mir Veröffentlichtes beanstandet werden, werden Sie niemals lesen, dass Claude es geschrieben hat. Der Text drückt aus, was ich denke und was ich geschrieben hätte, selbst dort, wo er überarbeitet wurde, und genau deshalb bleibt die letzte Kontrolle bei mir."
        },
        "h2_5": {
...
def main():
    ...
    if articles_to_process:
        ...
        for md_file in articles_to_process:
            pre_process_article_media(md_file, BASE_OUTPUT_DIR)
...
def process_article(md_file_info, output_dir_base, lang):
    ...
    try:
        ...
        for i, img in enumerate(soup.find_all('img')):
            ...
            processed_paths = process_and_save_image(original_src, output_dir_base, article_dir=md_file_info['parent_dir'])
...
def generate_article_pages(authors_data, articles, output_dir, lang='it', global_changed=True):
    ...
    for article in articles:
        ...
        if article.get('image_paths'):
             og_image = f"{SITE_URL}{article['image_paths']['full_jpeg']}"
        else:
...
def generate_author_pages(authors_data, articles, output_dir, lang='it'):
    ...
    for slug, author in authors_data.items():
        ...
        if author.get('photo'):
            photo_paths = process_author_photo(author['photo'], BASE_OUTPUT_DIR)
            if photo_paths:
                photo_tag = f"""
                <picture>
        ...
        og_image = f"{SITE_URL}{IMAGE_ASSETS_DIR}/authors/{os.path.basename(photo_paths['jpeg'])}" if author.get('photo') and photo_paths else f"{SITE_URL}logo_vn_ia.png"
        temp_html = temp_html.replace("{{og_url}}", og_url)
        temp_html = temp_html.replace("{{og_image}}", og_image)
