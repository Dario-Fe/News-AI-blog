...
def generate_content_pdf(articles, tocs, output_path, styles):
    ...
    author_image_path = os.path.join(REPO_ROOT, 'ebook/assets/autore.jpg')
    
    if os.path.exists(bio_path):
        # Aggiunge l'immagine dell'autore se esiste
        if os.path.exists(author_image_path):
            try:
                story.append(Spacer(1, 12*mm))
                # Logica di ridimensionamento per l'immagine dell'autore
                max_width = 80 * mm 
                max_height = 100 * mm
                pil_img = PILImage.open(author_image_path)
                original_width, original_height = pil_img.size
                ratio = min(max_width / original_width, max_height / original_height)
                new_width = original_width * ratio
                new_height = original_height * ratio
                
                img = Image(author_image_path, width=new_width, height=new_height, hAlign='CENTER')
                story.append(img)
                story.append(Spacer(1, 6*mm))
            except Exception as e:
                print(f"Attenzione: Impossibile processare l'immagine dell'autore {author_image_path}. Errore: {e}")

        # Aggiunge il testo della biografia
